	...
	Simulator<dim>::revise_composition_melt()
	{
         ...
         for (typename DoFHandler<dim>::active_cell_iterator cell = dof_handler.begin_active();
             ...
             if (cell->is_locally_owned())
             {
                 ...
                 {
                     unsigned int system_local_dof[3];
					 std::vector<double> composition_values(3);
					 std::vector<double> new_composition_value(3);
                     ...
                     //melt_solution(local_dof_indices[system_local_dof])=value;
					 for(unsigned j=0;j<3;j++)
