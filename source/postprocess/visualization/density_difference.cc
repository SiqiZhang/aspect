...
#include <aspect/postprocess/visualization/density_difference.h>
...
namespace aspect
{
  namespace Postprocess
  {
    ...
    {
      template <int dim>
      DensityDifference<dim>::
      DensityDifference ()
        :
        DataPostprocessorScalar<dim> ("density_difference",
                                         ...
                                         std::vector<Vector<double> >                    &computed_quantities) const
      {
        ...
        for (unsigned int q=0; q<n_quadrature_points; ++q)
        {
          in.temperature[q]=this->get_adiabatic_conditions().temperature(evaluation_points[q]);
          in.pressure[q]=this->get_adiabatic_conditions().pressure(evaluation_points[q]);
          for (unsigned int c=0; c<this->n_compositional_fields(); ++c)
            in.composition[q][c] = 0.;
        }
        this->get_material_model().evaluate(in, out);
        for (unsigned int q=0; q<n_quadrature_points; ++q)
          computed_quantities[q](0) -= out.densities[q];
...
namespace aspect
{
  namespace Postprocess
  {
    namespace VisualizationPostprocessors
    {
      ASPECT_REGISTER_VISUALIZATION_POSTPROCESSOR(DensityDifference,
                                                  "density_difference",
                                                  "A visualization output object that generates output "
                                                  "for the density difference compared to adiabtic density.")
    }
  }
}
